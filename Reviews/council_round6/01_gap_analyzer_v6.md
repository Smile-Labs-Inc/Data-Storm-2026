...
|---|---:|---:|---:|---|
| DE & Forensics (#1–5) | 40 | 8.0 | **32.0** | run_pipeline.py clean; SHA-256 full; TEAM_NAME correct. Lost pts: stale CSVs in gold/, 4 prediction CSVs in Results/, smil_labs_final_report.md still visible |
...
## N6.1 [HIGH] `Docs/smil_labs_final_report.md` — R5 said archive it, still present

R5 T1.3 explicitly said "Archive to `Docs/_archive/`". The file is still at `Docs/smil_labs_final_report.md` with:
...
**Fix:** Move to `Docs/_archive/smil_labs_final_report_v1.md`. Already documented in R5, just wasn't executed.
...
```
smil_labs_predictions.csv (361 KB) — canonical
smil_labs_predictions_full_20000.csv (361 KB) — duplicate
smil_labs_predictions_full_20000_v2.csv (361 KB) — duplicate
smil_labs_predictions_v2.csv (361 KB) — duplicate
```
...
**Fix:** Keep only `smil_labs_predictions.csv`. Move others to `Results/_legacy/`. Modify `write_submission()` to write only one file OR document why the full version exists.
...
|---:|---|---:|---:|---:|
| 1 | N6.1: Archive smil_labs_final_report.md | +1 pt (credibility) | 2 min | 30 pts/hr |
...
**`Docs/smil_labs_final_report.md` is still visible.** It claims 1.18× uplift and "Tobit Type-I MLE" — a method that doesn't exist in the codebase. If a judge opens `Docs/` and finds two conflicting reports, the credibility damage is severe and irreversible. This was flagged in R5 and not actioned. Fix it now.
