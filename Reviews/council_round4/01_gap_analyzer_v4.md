...
**Bottom line up front:** the team is sitting at a *low B / mid-70s* on paper but is **one accidental file pick away from disqualification** and the model is currently **failing two of its own auto-validations (V3b + V4)** — which is the single biggest score leak right now.
...
| **3** | **Submission-file hygiene** (rename + quarantine v1 files) | **+0 to +3 raw pts** but **prevents –100** (DQ avoidance) | **10 min** | **infinite (negative-tail)** | (a) `mv Results/smil_labs_predictions.csv Results/_legacy/v1_row_id_914rows.csv`; (b) `cp Results/smil_labs_predictions_v2.csv Results/smil_labs_predictions.csv`; (c) `mv Results/smil_labs_predictions_full_20000.csv Results/_legacy/`; (d) `head -1` the surviving file and confirm header is exactly `Outlet_ID,Maximum_Monthly_Liters`. |

> **Net of all 3:** ~+15 raw points in ~2 hours of work → score moves from **~66 → ~81** with the same model architecture, same POI scrape, same report. The lift is almost entirely *plumbing and renaming*.
...
- **Do regenerate the PDF** (or at minimum, edit the "auto-validation passes" verbatim block to match the *actual* `validation_report.md`). Submitting a PDF that claims PASS when the artifact says FAIL is the second-worst outcome after DQ.
...
**Hard rule for all three windows:** *NO new methods*. Every fix is "make what's already documented actually run".

---

# The One Unforced-Error Risk

> **`Results/` currently has FIVE prediction-looking files. The default-named one (`smil_labs_predictions.csv`) is the broken v1 with `row_id` as the column header and 914 rows. If anyone — judge, teammate, or upload-script — picks that file, the team is auto-disqualified per the official PDF schema requirement (`Outlet_ID, Maximum_Monthly_Liters`, 20,000 outlets in scope).**
...
Move-Item smil_labs_predictions.csv _legacy/v1_row_id_914rows.csv
Move-Item smil_labs_predictions_full_20000.csv _legacy/
Copy-Item smil_labs_predictions_v2.csv smil_labs_predictions.csv
Get-Content smil_labs_predictions.csv -TotalCount 2   # MUST print Outlet_ID,Maximum_Monthly_Liters
(Get-Content smil_labs_predictions.csv).Count          # MUST print 20001
```
