...
- **v1 artifacts are still polluting `data/gold/`** (`outlet_features.csv`, `validation_top_100_potential.csv`, `validation_top_100_uplift.csv`) and `Results/smil_labs_predictions.csv` is still in root — a judge who grabs the wrong file submits 914 rows with `row_id` headers.
...
| File | Issue | Action |
|------|-------|--------|
| `smil_labs_predictions.csv` | v1 — `row_id` column + 914 rows, still in root (R3 flagged as NOT TOUCHED) | Move to `Results/_legacy/` NOW |
| `smil_labs_predictions_full_20000.csv` | No `_v1` or `_v2` suffix — ambiguous | Rename to `_v1` or confirm it is a v2 artifact |
| `smil_labs_predictions_v2.csv` | v2 canonical | KEEP |
| `smil_labs_predictions_full_20000_v2.csv` | v2 | KEEP — includes full 20,000 rows |
...
- [ ] **Move `Results/smil_labs_predictions.csv` → `Results/_legacy/`** — risk of wrong-file upload
- [ ] **Rename or tag `Results/smil_labs_predictions_full_20000.csv`** — clarify v1 vs v2
...
| Path | Rationale |
|------|-----------|
| `src/` (*.py only, no __pycache__) | Full pipeline code |
...
| `Results/smil_labs_predictions_v2.csv` | **Canonical submission** |
| `Results/smil_labs_predictions_full_20000_v2.csv` | Full 20,000-row version |
