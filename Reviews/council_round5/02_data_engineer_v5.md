...
| ------------------------------------------ | ------ | ----------- | ------------------------------------------- | ---------------------------- |
| `smile_labs_predictions_research.csv`      | 17 KB  | today 20:07 | ~570 rows — NOT the 20k submission          | Move to `Results/_research/` |
| `smile_labs_predictions_research_full.csv` | 2.6 MB | today 20:07 | ~145k rows — multi-scenario research output | Move to `Results/_research/` |
...
# Produces: Results/smile_labs_predictions.csv  Results/smile_labs_predictions_full_20000.csv
```

The README, submission checklist, and every prior council review reference `smile_labs_predictions.csv`. Fresh pipeline run = wrong name.
...
[ ] 1 min  — run_pipeline.py:90:  TEAM_NAME = "smile_labs"
[ ] 5 min  — run_pipeline.py:124-129: SHA-256 full + column rename + filename
[ ] 2 min  — mkdir Results/_research/; mv smile_labs_predictions_research*.csv there
[ ] 5 min  — add one comment line to requirements_v2.txt re: conda-forge
[ ] Total: ~14 minutes
```

After these fixes: `python run_pipeline.py` produces `Results/smile_labs_predictions.csv` with V3b-safe rounding, full SHA-256 in `data/bronze/ingestion_audit.csv`, and no ambiguous files in `Results/`.
