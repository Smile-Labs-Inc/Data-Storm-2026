...
| `smile_labs_predictions.csv` correct headers + 20,000 rows | ✅ `head -1` confirmed                |
...
| N5.2  | `TEAM_NAME = "teamname"` → wrong submission filename                     | **CRITICAL** | `run_pipeline.py:90`      | Change to `"smile_labs"`                |
...
| N5.5  | `Docs/smile_labs_final_report.md`: 1.18× + "Tobit Type-I" (ghost method) | HIGH         | Docs/                     | Archive to `Docs/_archive/`             |
...
After: `python run_pipeline.py` → `Results/smile_labs_predictions.csv`, 6/6 PASS, full SHA-256 in audit.
...
### T1.3 — Archive `Docs/smile_labs_final_report.md` (~2 minutes)

Move to `Docs/_archive/smile_labs_final_report_v1.md`. No judge should see "Tobit Type-I" or "1.18×" alongside the canonical submission.
...
mv Results/smile_labs_predictions_research.csv Results/_research/
mv Results/smile_labs_predictions_research_full.csv Results/_research/
```
...
0:12 – 0:14  T1.3: Archive smile_labs_final_report.md
...
1:20 – 1:30  Visual verify smile_labs_predictions.csv header + row count
```
...
**6/6 PASS — submission is valid from `Results/smile_labs_predictions.csv` as of this round.**
