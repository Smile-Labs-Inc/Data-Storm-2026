...
\centering
\begin{tikzpicture}[
  layer/.style={draw, rounded corners, fill=primary!10, minimum width=2.2cm,
    minimum height=0.95cm, align=center, font=\small\bfseries},
  reject/.style={draw, rounded corners, fill=warn!12, minimum width=2.4cm,
    minimum height=0.85cm, align=center, font=\footnotesize},
  arrow/.style={-{Stealth[length=3mm]}, thick, primary!80}
]
\node[layer] (raw) {Raw CSVs};
\node[layer, right=1.4cm of raw] (bronze) {Bronze};
\node[layer, right=1.4cm of bronze] (silver) {Silver};
\node[layer, right=1.4cm of silver] (gold) {Gold};
\node[reject, below=0.7cm of silver] (rej) {Rejected\\(10{,}179 checks)};
\draw[arrow] (raw) -- node[above=2pt, font=\scriptsize]{ingest} (bronze);
\draw[arrow] (bronze) -- node[above=2pt, font=\scriptsize]{6 DQ checks} (silver);
\draw[arrow] (silver) -- node[above=2pt, font=\scriptsize]{features} (gold);
\draw[arrow] (silver) -- node[right=2pt, font=\scriptsize]{failures} (rej);
\end{tikzpicture}
\caption{Bronze $\to$ Silver $\to$ Gold pipeline with rejected-records store. The \emph{Rejected} branch is non-empty: every quarantined row keeps a \texttt{failure\_reason}.}
\end{figure}
...
The official PDF~\cite{datastorm2026pdf} explicitly judges \emph{``how robust
was the web-scraping / API pipeline for external POI data''}. Na\"ively
querying Overpass for $20{,}000 \times 9 \text{ cats} \times 4 \text{ radii}
= 720{,}000$ requests is infeasible (timeouts + ToS violation). We instead
download the \textbf{Geofabrik Sri Lanka country
extract}~\cite{geofabrik2026} (\texttt{sri-lanka-latest.osm.pbf},
$\sim$136~MB) once, parse it locally with
\texttt{pyrosm}~\cite{pyrosm2026, osm2026}, and run nearest-neighbour
searches with
\texttt{sklearn.neighbors.BallTree(metric="haversine")}~\cite{pedregosa2011sklearn}.
End-to-end: 25--45~min, entirely offline after the single fetch. The pipeline
lives in \texttt{poi\_pipeline/} as a separate, idempotent sub-project.
...
2.0}~\cite{chen2016xgboost} with \texttt{reg:quantileerror}~(quantile
regression~\cite{koenker1978qr}), \texttt{quantile\_alpha=[0.5, 0.75, 0.9,
0.95]}, monotone constraints + isotonic
post-sort~\cite{chernozhukov2010crossing}, and (b)
...
\textbf{Chernozhukov-Hong (2002)} 3-step~\cite{chernozhukov2002hong} (building
on Powell's censored quantile regression~\cite{powell1986censored}):
propensity $P(\text{censored}\mid X)$ fit on plateau / stuck-at-ceiling
proxies; q90 refit on $P<0.10$. Note: only 1.16\,\% of outlets show a true
...
calibrated $[q_{05}, q_{95}]$ via Conformalised QR~\cite{romano2019cqr}
(conformal-prediction framework of Vovk et al.~\cite{vovk2005algorithmic}) on
a 20\,\% outlet-level holdout (disjoint from the q90 training set after the
R4 fix), plus Manski worst-case bounds~\cite{manski2003partial} per outlet.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.6\textwidth]{figures/eda_manski_band.png}
\caption{Per-outlet Manski band width. Band lower edge = $\max(\text{lower\_bound}, \text{obs\_max})$;
upper edge = peer-q99 cap. \textbf{Sensitivity sweep:} across
quantile~$\{0.85, 0.9, 0.95\}$ $\times$ scheme~$\{$balanced, frontier-heavy,
plateau-heavy$\}$ $\times$ cap multiplier~$\{2..6\}$, median uplift stays at
\textbf{1.250$\times$} and cap-binding at $0\,\%$.}
\label{fig:manski}
\end{figure}
...
\midrule
Domain research    & 10-channel parallel research swarm (Claude Opus 4 + GPT-5.5) producing \texttt{research/research\_brief.md} & cross-checked each claim against cited URLs \\
...
Implementation     & Cursor + Claude/GPT routing scaffolded \texttt{src/quality}, \texttt{cleaning}, \texttt{features}, \texttt{modeling}, \texttt{reporting} & syntax + import check before commit; ReadLints clean \\
...
\bottomrule
\end{tabularx}
\caption{Per-phase GenAI transparency. Full log in \texttt{Docs/ai\_transparency\_log.md} (R1-R7 audit trail).}
\end{table}
...
\begin{tabularx}{\textwidth}{lX}
\toprule
\rowcolor{soft}\textbf{Artifact} & \textbf{Path (\texttt{Results/} or \texttt{Notebooks/})} \\
\midrulesmile
Platform submission CSV  & \texttt{Results/smile\_labs\_predictions.csv} (20{,}000 rows) \\
Full business output     & \texttt{Results/smil\_labs\_predictions\_full\_20000.csv} \\
Manski bands             & \texttt{Results/manski\_bands\_v2.csv} \\
Conformal intervals      & \texttt{Results/conformal\_intervals\_v2.csv} \\
Validation reports       & \texttt{Results/validation\_report.md} (6/6 PASS) + \texttt{validation\_extended\_v6.md} (V4b/V6/V7) \\
Reproducible codebase    & \texttt{run\_pipeline.py} one-shot \emph{or} \texttt{Notebooks/20..22\_v2\_*.ipynb}; modules under \texttt{src/} \\
\bottomrule
\end{tabularx}
\end{table}

% --- BIBLIOGRAPHY (only refs actually cited inline) -------------------------
% Suppress duplicate URL field when a DOI is already printed (hyperlinkable
% DOI is the canonical reference). Keeps the bibliography compact in 5 pages.
\AtEveryBibitem{%
