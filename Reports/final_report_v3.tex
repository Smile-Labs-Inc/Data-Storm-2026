% =============================================================================
% Smil Labs - Data Storm 7.0 (Storming Round) - Final Technical Report
%
...
% --- Colors (Smil Labs palette derived from John Keells blue) ----------------
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\textcolor{gray}{Smil Labs | Data Storm 7.0}}
...
{\large\bfseries Submitted by Smil Labs}\\[0.3em]
...
\centering
\begin{tikzpicture}[
  layer/.style={draw, rounded corners, fill=primary!10, minimum width=2.4cm,
...
\node[layer, right=0.8cm of raw] (bronze) {Bronze};
\node[layer, right=0.8cm of bronze] (silver) {Silver};
\node[layer, right=0.8cm of silver] (gold) {Gold};
\node[reject, below=0.5cm of silver] (rej) {Rejected\\(10{,}179 checks)};
\draw[arrow] (raw) -- (bronze);
\draw[arrow] (bronze) -- node[above, font=\scriptsize]{6 DQ checks} (silver);
\draw[arrow] (silver) -- node[above, font=\scriptsize]{features} (gold);
...
The official PDF explicitly judges \emph{``how robust was the web-scraping / API
pipeline for external POI data''}. Na\"ively querying Overpass for
$20{,}000 \times 9 \text{ cats} \times 4 \text{ radii} = 720{,}000$ requests
is infeasible (timeouts + ToS violation). We instead download the
\textbf{Geofabrik Sri Lanka country extract} (\texttt{sri-lanka-latest.osm.pbf},
$\sim$136~MB) once, parse it locally with \texttt{pyrosm}, and run
nearest-neighbour searches with \texttt{sklearn.neighbors.BallTree(metric="haversine")}.
...
\textbf{Frontier} = 60/40 blend of (a) \textbf{XGBoost 2.0} with
\texttt{reg:quantileerror}, \texttt{quantile\_alpha=[0.5, 0.75, 0.9, 0.95]},
monotone constraints + isotonic post-sort~\cite{chernozhukov2010crossing}, and
(b) \textbf{SFA}~\cite{aigner1977sfa}: $y = X\beta + v - u$ with
truncated-normal $u$, direct MLE; per-outlet
$\text{TE} = \exp(-\mathbb{E}[u\mid\varepsilon])$~\cite{jondrow1982je}
(fitted $\sigma_v \approx 0.41$, $\sigma_u \approx 0.78$, $\lambda \approx 1.92$;
median TE $\approx 0.84$). Leaky \texttt{observed\_*} aggregates dropped from SFA design matrix.
\end{methodbox}

\begin{methodbox}[title=\textbf{Censoring correction + constraint score}]
\textbf{Chernozhukov-Hong (2002)} 3-step~\cite{chernozhukov2002hong}: propensity
$P(\text{censored}\mid X)$ fit on plateau / stuck-at-ceiling proxies; q90 refit
on $P<0.10$. Note: only 1.16\,\% of outlets show a true plateau fingerprint
globally (primarily \texttt{DIST\_S\_01}, \texttt{DIST\_S\_02}); CH-3 is a
small-population correction, not a load-bearing transform.
...
\item \textbf{Platform CSV:} \texttt{Results/smil\_labs\_predictions.csv}
\item \textbf{Full business output:} \texttt{Results/smil\_labs\_predictions\_full\_20000.csv}
\item \textbf{Manski bands:} \texttt{Results/manski\_bands\_v2.csv}
\item \textbf{Conformal intervals:} \texttt{Results/conformal\_intervals\_v2.csv}
\item \textbf{Reproducible codebase:} \texttt{Notebooks/20\_..\_pipeline.ipynb} $\to$ \texttt{21\_..\_modeling.ipynb} $\to$ \texttt{22\_..\_validation.ipynb}; modules in \texttt{src/}.
\end{itemize}
\end{multicols}
